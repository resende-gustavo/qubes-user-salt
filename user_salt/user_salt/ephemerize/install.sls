{% set default_kernel = salt['cmd.run']('qubes-prefs default-kernel') %}

path-kernel-initramfs:
  file.managed:
    - name: /var/lib/qubes/vm-kernels/patch_initramfs.sh
    - mode: 755
    - user: root
    - makedirs: True
    - contents: |
...
duplicate-default-kernel:
  cmd.run:
    - name: cp -r {{ default_kernel }} {{ default_kernel }}-ephemeral
    - cwd: /var/lib/qubes/vm-kernels/
    - runas: root
    
patch-duplicated-kernel:
  cmd.run:
    - name: sh patch_initramfs.sh {{ default_kernel }}-ephemeral
    - cwd: /var/lib/qubes/vm-kernels/
    - runas: root

remove-script:
  cmd.run:
    - name: rm patch_initramfs.sh
    - cwd: /var/lib/qubes/vm-kernels/
    - runas: root

set-ephemeral:
  file.managed:
    - name: /home/user/.local/bin/qvm-ephemeral
    - user: user
    - mode: 755
    - makedirs: True
    - contents: |
        qvm-volume config $1:root rw 0
        qvm-volume config $1:private rw 0
        qvm-volume config $1:volatile ephemeral 1
